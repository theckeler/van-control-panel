...
import { Modal, SelectableTile } from "../ui";
...
function socColor(soc: number) {
  if (soc > 50) return colors.lime[500];
  if (soc > 20) return colors.amber[500];
...
export function HistoryCard({
    ...
    : (() => {
        ...
        solarRaw
          ...
          .forEach((r) => {
            ...
            buckets.set(
              bucket,
              Math.max(buckets.get(bucket) ?? 0, r.solar_power as number),
            );
          });
  ...
  const chartColor = socColor(
    currentSoc ?? socData[socData.length - 1]?.soc ?? 99,
  );
    ...
    <Modal open={open}>
      <div className={className}>
        <div className="flex items-center justify-between mb-4 w-full">
          <span className="text-gray-800 uppercase">History</span>
