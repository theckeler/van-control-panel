...
function buildSeries(
  ...
  for (let i = 0; i * stepH <= hoursBack; i++) {
    ...
    soc = Math.max(
      18,
      Math.min(100, soc + ((netW * stepH) / BATTERY_WH) * 100),
    );
...
function monthlyReadings(): DailyReading[] {
  ...
  for (let m = 11; m >= 0; m--) {
    ...
    const seasonal =
      0.45 + 0.55 * Math.sin(((month - 2) / 12) * 2 * Math.PI) ** 2;
...
export const mockApi = {
  battery: {
    get: (): Promise<BatteryData> => {
      ...
      return delay({
        ...
        temperature: p.temp,
        cell_voltages: [0, 1, 2, 3].map(
          (i) => +(p.voltage / 4 + (i - 1.5) * 0.002).toFixed(3),
        ),
        cycle_count: 47,
        status: bmsReleased
          ? "released"
    ...
    release: () => {
      bmsReleased = true;
      return delay({ status: "ok", message: "BMS released (demo)" });
    },
    connect: () => {
      bmsReleased = false;
      return delay({ status: "ok", message: "BMS reconnected (demo)" });
    },
    history: {
  ...
  mppt: {
    get: (): Promise<MpptData> => {
      ...
      return delay({
        panel_voltage: +(p.solar > 5 ? 18.4 + Math.random() * 0.6 : 0).toFixed(
          1,
        ),
  ...
  shelly: {
    ...
    get: (id: string): Promise<ShellyUnit> =>
      delay({ ...shellys.find((s) => s.id === id)! }),
  ...
  system: {
    ...
    switchWifi: (name: string) =>
      delay({
        ok: false,
        message: `Switching to ${name} is disabled in demo mode`,
      }),
    ...
    shutdown: () =>
      delay({ status: "demo", message: "Shutdown disabled in demo mode" }),
    reboot: () =>
      delay({ status: "demo", message: "Reboot disabled in demo mode" }),
    diskImageStart: () =>
      delay({ ok: false, message: "Disk image disabled in demo mode" }),
    diskImageStatus: () =>
      delay({ state: null, bytes_written: null, filename: null, error: null }),
    diskImageUrl: () => "",
